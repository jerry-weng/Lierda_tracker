...
{
    MEMS_I2C_ReadAddr(Reg, Data);
    //MEMS_SPI_ReadAddr(Reg|0x80, Data);
    return 1;
}
...
uint8_t LIS3DH_WriteReg(uint8_t WriteAddr, uint8_t Data)
{
  ...
  //i.e. SPI_Mems_Write_Reg(WriteAddr, Data);
    //MEMS_SPI_WriteAddr(WriteAddr, Data);
    
    MEMS_I2C_WriteAddr(WriteAddr, Data);
    return 1;
}
...
/*******************************************************************************
* Function Name  : LIS3DH_SetBoot
* Description    : Reset sensor
* Input          : None
* Output         : None
* Return         : Status [MEMS_ERROR, MEMS_SUCCESS]
*******************************************************************************/
status_t LIS3DH_SetBoot(void)
{
    uint8_t value;
...
status_t LIS3DH_SetActTHS(uint8_t val)
{
    if (!LIS3DH_WriteReg(0x3E, val))
        return MEMS_ERROR;

    return MEMS_SUCCESS;
}

status_t LIS3DH_SetActDur(uint8_t val)
{
    if (!LIS3DH_WriteReg(0x3F, val))
        return MEMS_ERROR;
