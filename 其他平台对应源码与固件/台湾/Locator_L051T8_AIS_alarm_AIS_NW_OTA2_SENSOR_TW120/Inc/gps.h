...
#define SCHEDULE_GPS_NUM_MAX              20          //最大保存20组GPS数据
...
{
    uint8_t state;                               //执行结果
    uint8_t Eff_satellites_Num;                  //有效卫星数 
    uint8_t Scanned_satellites_Num;              //总卫星数
    uint8_t SNR30up_satellites_Num;              //SNR>30卫星数
}GPS_GSV_Data_t;
...
typedef struct
{
    uint8_t gsv_sum;    //语句总数
    uint8_t gsv_num;    //语句号
    uint8_t sat_num;    //卫星总数
    GPS_SAT_Info_t GPS_SAT_Info[30]; //卫星信息
}GPS_GSV_Info_t;
