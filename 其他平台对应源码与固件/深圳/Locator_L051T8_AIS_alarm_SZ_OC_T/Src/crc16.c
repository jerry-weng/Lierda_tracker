...
bool do_crc_value(unsigned char* message,unsigned char* crc,int lenth)
{
	int crc_reg = 0x0000;
	int crc_value = (crc[0]&0xff)*256+(crc[1]&0xff);
	if(crc_value == do_crc(crc_reg,message,lenth))
	{
		return true;
	}
	else
...
bool do_crc_db44(unsigned char * message,int lenth)
{
	unsigned char *messageArray = NULL;
	unsigned char crcArray[2];
	strncpy((char*)messageArray, (char*)message,lenth-2);
	crcArray[0] = message[lenth-2];
	crcArray[1] = message[lenth-1];
	return do_crc_value(messageArray,crcArray,lenth);
}
...
//	int crc = 0;
//	int i = 0;
//	char str[] = "FFFE0113000000110056302E30000000000000000000000000";
//
//	HexStrToByte(str,test,strlen(str));
//	for(i = 0;i<strlen(str)/2;i++)
//	{
//		printf("%02X ",test[i]);
//	}
//	
//	crc = crc_check(test,strlen(str)/2);
//	printf("\n%04X\n",crc);
//	return 0;
//}
